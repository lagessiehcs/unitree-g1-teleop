...
CORRECTIONS = {                         # XYZ
    SensorID.LowerBack: [0.0, 0.0, 0.0], 
    SensorID.UpperBack: [0.0, 0.0, 0.0],

    SensorID.ThighLeft: [0.0, 0.0, 0.0],
    SensorID.ThighRight: [0.0, 0.0, 0.0],
    SensorID.ShankLeft: [0.0, 0.0, 0.0],
    SensorID.ShankRight: [0.0, 0.0, 0.0],
    SensorID.FootLeft: [0.0, 0.0, 0.0],
    SensorID.FootRight: [0.0, 0.0, 0.0],

    SensorID.ShoulderLeft: [0.0, 0.0, 0.0],
    SensorID.ShoulderRight: [0.0, 0.0, 0.0],
    SensorID.UpperArmLeft: [0.0, 0.0, 0.0],
    SensorID.UpperArmRight: [0.0, 0.0, 0.0],
    SensorID.ForearmLeft: [0.0, 0.0, 0.0],
    SensorID.ForearmRight: [0.0, 0.0, 0.0],
    SensorID.HandLeft: [0.0, 0.0, 0.0],
    SensorID.HandRight: [0.0, 0.0, 0.0],
}
# Automatically generated rotations
SENSORSUIT_CALIBRATION_CORRECTIONS = {
    k: R.from_euler('XYZ', v, degrees=True)
    for k, v in CORRECTIONS.items()
}

# Automatically generated inverse rotations
SENSORSUIT_ROTATION_RESTORATIONS = {
    k: R.from_euler('XYZ', -np.array(v), degrees=True)
    for k, v in CORRECTIONS.items()
}
class G1TeleopNode(Node):
    ...
    def __init__(self, args):     
        ...
        self.offset_orientation = {
            SensorID.ThighLeft:     R.from_euler('XYZ', [0.0, -90.0, 0.0], degrees=True),
            ...
            SensorID.ThighRight:    R.from_euler('XYZ', [0.0, 90.0, 0.0], degrees=True),
            ...
            SensorID.LowerBack:     R.from_euler('XYZ', [0.0, 0.0, 0.0], degrees=True),
        ...
        path = args.save_file_path

        self.record_thread = threading.Thread(target=self.record, args=(path,))
    ...
    def calibrate(self, sensor_orientations):
        ...
        for link_id, base_id in self.link_id_to_base_id_mapping.items():
                ...
                self.calib_orientations[link_id] = (
                sensor_orientations[base_id] *
                SENSORSUIT_CALIBRATION_CORRECTIONS[base_id].inv() *
                self.offset_orientation[link_id] *
                SENSORSUIT_CALIBRATION_CORRECTIONS[link_id] *
                sensor_orientations[link_id].inv()
            )
    ...
    def record(self, file_path):
        ...
        if dir_path:  # avoids issues if only filename is given
            os.makedirs(dir_path, exist_ok=True)

        with open(file_path, "w") as f:
            record = {"upperbody": self.upperbody}
    ...
    def listener_callback(self, msg):
        ...
        for link_id, joint_id_group in self.link_id_to_joint_id_group_mapping.items():
            if self.upperbody or self.teleop_enabled:
                ...
                joint_rotation = SENSORSUIT_ROTATION_RESTORATIONS[base_id].inv() * self.sensor_orientations[base_id].inv() * self.calib_orientations[link_id] * self.sensor_orientations[link_id] * SENSORSUIT_ROTATION_RESTORATIONS[link_id]
